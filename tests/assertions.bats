...
@test "assert_equal succeeds for identical strings" {
  run bash -c 'source tests/test_helper/bats-assert/load; assert_equal "foo" "foo"'
  assert_success
}

@test "assert_equal fails and shows diff for different multiline values" {
  run bash -c 'source tests/test_helper/bats-assert/load; assert_equal $'"'"'line1\nline2\n'"'"' $'"'"'line1\nlineX\n'"'"''
  assert_failure
...
@test "assert_not_equal succeeds for different strings" {
  run bash -c 'source tests/test_helper/bats-assert/load; assert_not_equal "foo" "bar"'
  assert_success
}

@test "assert_not_equal fails for equal strings" {
  run bash -c 'source tests/test_helper/bats-assert/load; assert_not_equal "foo" "foo"'
  assert_failure
...
@test "assert_json_equal ignores key order" {
  local a='{"b":1,"a":2}'
  local b='{"a":2,"b":1}'
  run bash -c 'source tests/test_helper/bats-assert/load; assert_json_equal "$1" "$2"' _ "$a" "$b"
  assert_success
}
...
@test "assert_json_not_equal succeeds on difference" {
  local a='{"x":1}'
  local b='{"x":2}'
  run bash -c 'source tests/test_helper/bats-assert/load; assert_json_not_equal "$1" "$2"' _ "$a" "$b"
  assert_success
}
...
@test "_json_normalize works with jq or python3" {
  local j='{"z":1,"a":2}'
  run bash -c 'source tests/test_helper/bats-assert/load; _json_normalize <<<"$1"' _ "$j"
  assert_success
...
@test "assert_json_equal reports invalid JSON" {
  local bad='{"a":1'
  run bash -c 'source tests/test_helper/bats-assert/load; assert_json_equal "$1" "$2"' _ "$bad" '{"a":1}'
  assert_failure
  [[ "$output" == *"invalid"* ]]
}
...
@test "assert_json_not_equal reports invalid JSON" {
  local bad='[1,2'
  run bash -c 'source tests/test_helper/bats-assert/load; assert_json_not_equal "$1" "$2"' _ "$bad" '[1,2,3]'
  assert_failure
  [[ "$output" == *"invalid"* ]]
}
...
@test "assertions library loads cleanly" {
  run bash -c 'source tests/test_helper/bats-assert/load; echo OK'
  assert_success
  assert_output "OK"
}
