...
wgx_run_subcommand(){
  ...
  local cmd_file
  cmd_file="$(wgx_resolve_subcommand_file "$sub" 2>/dev/null || true)"
  if [[ -n "$cmd_file" ]]; then
    unset -f wgx_command_main >/dev/null 2>&1 || true
    # shellcheck disable=SC1090
    source "$cmd_file"
    if declare -f wgx_command_main >/dev/null 2>&1; then
      wgx_command_main "$@"
      return $?
    fi
    die "cmd/${sub}.bash definiert keine wgx_command_main"
  fi
...
wgx_main(){
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --timeout)
        shift
        [[ "${1-}" =~ ^[0-9]+$ ]] || die "--timeout braucht Zahl"
        TIMEOUT="$1"
        ;;
